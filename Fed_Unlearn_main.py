...
from non_iid_data import data_init_non_iid, data_init_dirichlet
...
class Arguments:
    def __init__(self):
        ...
        self.N_client = (
            10  # clients that are selected to participate in the unlearning process
        )
        ...
        self.split_non_iid = "dirichlet"  # "dirichlet" or "class"
...
def Federated_Unlearning():
    ...
    if FL_params.split_non_iid == "dirichlet":
        client_all_loaders, test_loader = data_init_dirichlet(FL_params)
    elif FL_params.split_non_iid == "class":
        client_all_loaders, test_loader = data_init_non_iid(FL_params)
    else:
        client_all_loaders, test_loader = data_init(FL_params)
    logger.debug(f"client_all_loaders: {len(client_all_loaders)}")
    logger.debug(f"test_loader: {len(test_loader)}")
    ...
    logger.debug(f"ACC_old: {ACC_old}, PRE_old: {PRE_old}")

    if FL_params.if_retrain == True:
        print("Attacking against FL Retrain  ")
        target_model = retrain_GMs[T_epoch]
        (ACC_retrain, PRE_retrain) = attack(
            target_model, attack_model, client_loaders, test_loader, FL_params
        )
        logger.debug(f"ACC_retrain: {ACC_retrain}, PRE_retrain: {PRE_retrain}")
    ...
    logger.debug(f"ACC_unlearn: {ACC_unlearn}, PRE_unlearn: {PRE_unlearn}")
